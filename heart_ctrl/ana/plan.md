...
1. Pointwise Convolution (逐点卷积)Pointwise Conv 其实就是卷积核大小为 $1 \times 1$ 的标准卷积。操作方式： 它在每个时间点（或空间点）上，对所有通道的数值进行加权求和。在时序中的作用：通道融合： 它不关注时间前后的变化，只负责整合当前时刻下不同特征（变量）之间的关系。维度变换： 最常用的功能是升维或降维。例如，将输入的 64 个特征维度压缩到 32 个，或者扩展到 128 个以匹配后续层的输入需求。2. 1D Depthwise Convolution (一维深度卷积)Depthwise Conv 是一种“专款专用”的卷积，它对每个通道独立进行卷积操作。操作方式： 如果输入有 $C$ 个通道，Depthwise Conv 就会有 $C$ 个独立的 $1 \times K$（$K$ 是卷积核大小）的一维卷积核。每个核只负责处理自己对应的那个通道，通道之间互不干扰。在时序中的作用：时间特征提取： 它专门负责捕捉时间维度上的局部模式（比如过去 31 天的趋势）。极低功耗： 相比标准卷积，它的计算量减少了 $1/C$。在长序列任务中，这能显著降低显存占用
